...
class XpCache extends phpFastCache
{
    protected static $instances = array();
    public static function & instance($storage, $config_group = 'config')
    {
        if (!isset(XpCache::$instances[$storage]))
        {
            XpCache::$instances[$storage] = new XpCache($storage,$config_group);
        }

        return XpCache::$instances[$storage];
    }

    public function  __construct($storage = FALSE, $config_group)
    {
        global ${$config_group};

        $config_group = ${$config_group};
        if (is_array($config_group[$storage]))
        {
            self::$server = $config_group[$storage];
            self::$storage = $storage;
        }
        self::$path = APP_FOLDER.'/'.$config_group['cacheDirName'];
    }
...
class phpFastCache
{
    ...
    public static $securityKey = "storage"; // phpFastCache::$securityKey = "newKey";
...
allow from 127.0.0.1";
        ...
        if(self::$objects['memcache'] == "")
        {
            ...
            foreach(self::$server as $server)
            {
                ...
                if(!in_array($name, self::$checked['memcache']) && $name !="") {
                    self::$objects['memcache']->addServer($name,$port,$weight);
    ...
    private static function memcache_set($name,$value,$time_in_second = 600, $skip_if_existing = false)
    {
        ...
        {
            return self::$objects['memcache']->set($name, $value, false, time() + $time_in_second );
        }
        else
        {
            return self::$objects['memcache']->add($name, $value, false, time() + $time_in_second );
        }
    }
    ...
    public static function memcached_addserver()
    {
        ...
        if(self::$objects['memcached'] == "")
        {
            ...
            foreach(self::$server as $server)
            {
                ...
                if(!in_array($name, self::$checked['memcached']) && $name !="")
                {
                    if($weight >0 ) {
                        self::$objects['memcached']->addServer($name,$port,$weight);
                    } else {
