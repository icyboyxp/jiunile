...
function create_captcha($data = '', $img_path = '', $img_url = '')
{
    ...
    while ($filename = @readdir($current_dir))
    {
        if ($filename != "." and $filename != ".." and $filename != "index.html")
        {
            ...
            if ( ($name + $expiration) < $now ) @unlink($img_path.$filename);
        }
    }
    ...
    for($i=0;$i<32;$i++)
    {
        ...
        if($i>0)
        {
            ...
            for($k=0;$k<strlen($word);$k++)
            {
                ...
                $float_top=rand(0,4);
                //$float_left=rand(0,3);
                imagestring($image,6,$space*$k,$top+$float_top,substr($word,$k,1),$colorList[$colorRandom]);
            }
        }
        ...
        for($k=0;$k<3;$k++)
        {
            ...
            $todrawline=1;
            if($todrawline){
                imageline($image,mt_rand(0,$img_width),mt_rand(0,$img_height),mt_rand(0,$img_width),mt_rand(0,$img_height),$colorList[$colorRandom]);
            }else{
                $w=mt_rand(0,$img_width);
                $h=mt_rand(0,$img_width);
                imagearc($image,$img_width-floor($w / 2),floor($h / 2),$w,$h, rand(90,180),rand(180,270),$colorList[$colorRandom]);
            }
        }
